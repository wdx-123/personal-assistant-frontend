...
export const isPermissionDenied = (error: unknown): boolean => {
  ...
  return (
    status === 403 ||
    normalizedCode === StatusCode.FORBIDDEN ||
    normalizedCode === BizCode.CodePermissionDenied
  );
};

const syncAbortSignals = (
  sourceSignal: GenericAbortSignal | undefined,
...
const refreshAccessToken = async (): Promise<string> => {
  ...
  const { access_token, access_token_expires_at } = await refreshToken(
    refreshTokenValue,
    {
      skipTip: true,
      skipSuccTip: true,
...
const responseInterceptor = (
  ...
  const { skipTip, skipSuccTip, skipErrTip, customSuccTip, customErrTip } =
    config;
  const normalizedCode =
    typeof res.code === "string" ? Number(res.code) : res.code;
  ...
  if (
    normalizedCode === StatusCode.UNAUTHORIZED ||
