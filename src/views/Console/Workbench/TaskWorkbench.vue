...
type FormItemAnalysisStatus = 'idle' | 'analyzing' | 'resolved' | 'ambiguous' | 'missing' | 'invalid'
...
const STABLE_EXECUTION_STATUS = new Set<OJTaskExecutionStatus>(['succeeded', 'failed', 'cancelled'])
...
const TASK_STATUS_FILTER_OPTIONS: Array<{ label: string; value: TaskStatusFilterValue }> = [
  { label: '全部状态', value: 'all' },
  ...TASK_STATUS_OPTIONS,
]

const EXECUTION_STATUS_META: Record<OJTaskExecutionStatus, { label: string; color: string }> = {
...
const FORM_ANALYSIS_META: Record<FormItemAnalysisStatus, { label: string; color: string }> = {
...
const RESOLUTION_STATUS_META: Record<OJTaskItemResolutionStatus, { label: string; color: string }> = {
  resolved: { label: '已解析', color: 'success' },
...
const taskFormMissingRiskCount = computed(() => taskFormAnalysisSummary.value.missing)
...
const taskFormAnalysisMessage = computed(() => {
  ...
  if (taskFormBlockingCount.value === 0 && taskFormMissingRiskCount.value === 0) {
  ...
  if (taskFormAnalysisSummary.value.ambiguous) {
    blockingParts.push(`${taskFormAnalysisSummary.value.ambiguous} 道待选择候选`)
  }
  if (taskFormAnalysisSummary.value.invalid) {
    blockingParts.push(`${taskFormAnalysisSummary.value.invalid} 道需要修正输入`)
  }
...
const taskFormReady = computed(() => {
  ...
  if (taskForm.items.length === 0 || !taskForm.items.every((item) => isTaskItemReadyForSubmit(item))) {
    return false
  }
...
function buildTaskActionStateFromRecord(record: Record<string, unknown>): TaskActionState {
...
function openTaskDetailFromRecord(record: Record<string, unknown>): void {
  void openTaskDetail(Number(record.task_id || 0), Number(record.execution_id || 0))
}
...
function openExecutionUserDetailFromRecord(record: Record<string, unknown>): void {
...
function handleTaskItemPlatformChange(item: TaskFormItem, value: OJPlatform): void {
  item.platform = value
...
async function analyzeTaskItems(options: {
  indexes?: number[]
  ...
  const targetIndexes = Array.from(
    new Set(
      (options.indexes?.length ? options.indexes : taskForm.items.map((_, index) => index))
  ...
  try {
    ...
    analysis.missing.forEach((missingItem) => {
      ...
      target.errorText = '本地题库未命中。你可以修正标题后重新分析，也可以继续提交为待解析状态。'
    ...
    const allSubmitReady = taskForm.items.every((item) => isTaskItemReadyForSubmit(item))
...
async function executeTaskFormSubmit(): Promise<void> {
  ...
  try {
    ...
    if (taskDrawerMode.value === 'edit') {
      ...
      if (detailDrawerOpen.value && detailTask.value?.task_id === editingTaskId.value) {
...
async function loadTaskVersions(taskId: number, options: { silent?: boolean } = {}): Promise<void> {
...
async function refreshTaskDetail(options: { silent?: boolean } = {}): Promise<void> {
  if (!detailTask.value) {
    return
  }
...
async function refreshDetailPanel(options: { silent?: boolean } = {}): Promise<void> {
  if (!detailTask.value) {
    return
  }
...
async function loadExecutionUsers(options: { silent?: boolean } = {}): Promise<void> {
...
async function handleMutationResult(result: OJTaskCreateResponse, successMessage: string): Promise<void> {
  taskDrawerOpen.value = false
  await fetchTasks({ silent: true })
...
async function submitTaskForm(): Promise<void> {
  if (taskDrawerLoading.value || taskSubmitting.value || taskSubmitConfirming.value || taskAnalyzing.value) {
    return
  }
...
<template>
  <div class="page-container">
    <a-card :bordered="false" class="search-card" :body-style="{ padding: '24px 24px 0 24px' }">
      <div class="task-search-layout">
        <a-form layout="inline" class="search-form task-search-main">
          <a-form-item label="关键词" class="task-search-item task-search-item-keyword">
          <a-input
          ...
          <a-form-item label="版本视图" class="task-search-item task-search-item-version">
          <a-radio-group
        ...
        <template #bodyCell="{ column, record }">
          ...
          <template v-else-if="column.key === 'action'">
            <div class="action-buttons">
              <a-button type="link" size="small" @click="openTaskDetailFromRecord(record)">
              ...
              <a-button
                ...
                type="link"
                size="small"
                @click="confirmExecuteTask(buildTaskActionStateFromRecord(record))"
              >
              ...
              <a-button
                ...
                type="link"
                size="small"
                @click="confirmRetryTask(buildTaskActionStateFromRecord(record))"
              >
              ...
              <a-button
                ...
                danger
                size="small"
                @click="confirmDeleteTask(buildTaskActionStateFromRecord(record))"
              >
      ...
      <a-spin :spinning="taskDrawerLoading || taskSubmitting">
        <a-form layout="vertical" class="task-form">
          <a-row :gutter="16">
            ...
            <a-col :xs="24" :lg="10">
              <a-form-item label="执行模式" required>
                <a-radio-group v-model:value="taskForm.mode" :disabled="taskDrawerMode === 'edit'">
          ...
          <a-card :bordered="false" class="analysis-summary-card">
            <div class="analysis-summary-header">
              ...
              <a-space>
                <a-button
                  ...
                  @click="taskAnalyzing = true; analyzeAllTaskItems('manual').finally(() => (taskAnalyzing = false))"
                >
            ...
            <a-space wrap>
              <a-tag color="success">已解析 {{ taskFormAnalysisSummary.resolved }}</a-tag>
              <a-tag color="processing">分析中 {{ taskFormAnalysisSummary.analyzing }}</a-tag>
              <a-tag color="default">待分析 {{ taskFormAnalysisSummary.idle }}</a-tag>
              <a-tag color="warning">待选择 {{ taskFormAnalysisSummary.ambiguous }}</a-tag>
              <a-tag color="warning">未命中 {{ taskFormAnalysisSummary.missing }}</a-tag>
              <a-tag color="volcano">需修正 {{ taskFormAnalysisSummary.invalid }}</a-tag>
            </a-space>

            <a-alert
              :type="taskFormBlockingCount === 0 && taskFormMissingRiskCount === 0 ? 'success' : 'warning'"
              show-icon
            ...
            >
              ...
              <div class="task-item-row">
                <a-select
                  ...
                  @update:value="(value) => handleTaskItemPlatformChange(item, value as OJPlatform)"
                />
                <a-input
                  :value="item.title"
                  class="task-item-title-input"
                  placeholder="请输入题目标题，例如 Two Sum / A+B Problem"
                  @update:value="(value) => handleTaskItemTitleChange(item, value as string)"
                />
              </div>
                ...
                <template v-else-if="item.status === 'missing'">
                  <div class="analysis-feedback-main">
                    ...
                    <div>
                      ...
                      <div class="analysis-feedback-subtitle">
                        {{ item.errorText || '可修正标题后重新分析，也可继续提交为待解析状态，后续由系统自动回填。' }}
                      </div>
                    </div>
                  </div>
                </template>
        ...
        <div class="drawer-footer">
          <a-space>
            ...
            <a-button
              ...
              :disabled="taskDrawerLoading || taskSubmitConfirming || taskAnalyzing || !taskFormReady"
    ...
    >
      <template #title>
        <div class="detail-title-block">
          <div class="detail-title-main">{{ detailTask?.title || '任务详情' }}</div>
          <div class="detail-title-sub">
            <span v-if="detailTask">v{{ detailTask.version_no }} · task #{{ detailTask.task_id }}</span>
            <span v-if="detailTask?.root_task_id"> · root #{{ detailTask.root_task_id }}</span>
            <span v-if="currentExecution"> · execution #{{ currentExecution.execution_id }}</span>
          </div>
        </div>
      </template>
            ...
            <a-tab-pane key="overview" tab="任务概览">
              ...
              <div class="stats-grid">
                ...
                <a-card class="stat-card">
                  ...
                  <div class="stat-value">
                    <a-tag :color="getExecutionStatusMeta(currentExecution.status).color">
                ...
                <a-card class="stat-card">
                  ...
                  <div class="stat-number">{{ currentExecution.total_user_count }}</div>
                </a-card>
                <a-card class="stat-card">
                  <div class="stat-label">已完成用户</div>
                  <div class="stat-number">{{ currentExecution.completed_user_count }}</div>
                </a-card>
                <a-card class="stat-card">
                  <div class="stat-label">待完成用户</div>
                  <div class="stat-number">{{ currentExecution.pending_user_count }}</div>
                </a-card>
                <a-card class="stat-card">
              ...
              <a-card :bordered="false" class="section-card">
                <a-descriptions :column="2" bordered class="detail-descriptions">
                  ...
                  <a-descriptions-item label="计划执行时间">
                    {{ formatDateTime(detailTask.execute_at || currentExecution.planned_at) }}
                  </a-descriptions-item>
                  <a-descriptions-item label="请求人 ID">
                    <span class="mono-text">{{ currentExecution.requested_by }}</span>
                  </a-descriptions-item>
                  ...
                  <a-descriptions-item label="执行组织" :span="2">
                    <a-space wrap>
                      <a-tag v-for="org in detailTask.orgs" :key="org.org_id" color="blue">
                        {{ org.org_name }}
                      </a-tag>
                    </a-space>
                  </a-descriptions-item>
              ...
              <a-card :bordered="false" class="section-card" title="任务题单快照">
                <a-table
              ...
              <a-spin :spinning="versionLoading">
                <div v-if="taskVersions.length" class="version-list">
                  <div
                    ...
                    :class="[
                      'version-item',
                      { active: version.task_id === detailTask.task_id && version.execution_id === currentExecution.execution_id },
                    ]"
                  >
                    <div class="version-item-main">
                      ...
                      <div class="version-item-meta">
                        <a-tag :color="version.mode === 'scheduled' ? 'gold' : 'blue'">
                          {{ getModeLabel(version.mode) }}
                        </a-tag>
                        <a-tag :color="getTaskStatusMeta(version.status).color">
                          {{ getTaskStatusMeta(version.status).label }}
                        </a-tag>
                        <a-tag :color="getExecutionStatusMeta(version.execution_status).color">
                          {{ getExecutionStatusMeta(version.execution_status).label }}
                        </a-tag>
                      </div>
                      <div class="version-item-sub">
                        task #{{ version.task_id }} · execution #{{ version.execution_id }} ·
                    ...
                    <a-button
                      type="primary"
                      ghost
                      :disabled="version.task_id === detailTask.task_id && version.execution_id === currentExecution.execution_id"
            ...
            <a-tab-pane key="users" tab="执行用户">
              <a-card :bordered="false" class="search-card users-search-card" :body-style="{ padding: '16px 16px 0 16px' }">
                <a-form layout="inline">
                  ...
                  <a-form-item label="完成范围">
                    <a-select v-model:value="userFilters.completion_scope" style="width: 160px">
                  ...
                  <a-form-item>
                    <a-space>
                      <a-button type="primary" @click="handleExecutionUserSearch">
                        <template #icon><SearchOutlined /></template>
                        查询
                      </a-button>
              ...
              >
                <template #bodyCell="{ column, record }">
                  ...
                  <template v-else-if="column.key === 'action'">
                    <a-button type="link" size="small" @click="openExecutionUserDetailFromRecord(record)">
                      查看
                    </a-button>
                  </template>
                </template>
    ...
    >
      <template #title>
        <div class="detail-title-block">
          <div class="detail-title-main">{{ executionUserDetail?.username_snapshot || '用户执行快照' }}</div>
          <div class="detail-title-sub">
            <span v-if="executionUserDetail">用户 #{{ executionUserDetail.user_id }}</span>
          </div>
        </div>
      </template>
        ...
        <template v-if="executionUserDetail">
          <a-card :bordered="false" class="section-card">
            <a-descriptions :column="2" bordered class="detail-descriptions">
              ...
              <a-descriptions-item label="执行组织" :span="2">
                <a-space wrap>
                  <a-tag v-for="org in executionUserDetail.orgs" :key="`${executionUserDetail.user_id}-${org.org_id}`">
                    {{ org.org_name_snapshot }}
                  </a-tag>
                </a-space>
              </a-descriptions-item>
